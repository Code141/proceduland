...
getAndresLenght = function(rayon, x_centre, y_centre, fill){
	andresLenght = function( r, x_centre, y_centre ){
		x = 0;
		y = r
		d = r - 1;
		while( y >=x ){
			addLenght( x_centre + x , y_centre - y ); // NORTH - EAST // 1H
			addLenght( x_centre + y , y_centre - x );	// EAST - NORTH // 2H
			addLenght( x_centre + y , y_centre + x ); // EAST - SOUTH // 4H
			addLenght( x_centre + x , y_centre + y ); // SOUTH - EAST // 5H
			addLenght( x_centre - x , y_centre + y ); // SOUTH - WEST // 7H
			addLenght( x_centre - y , y_centre + x );	// WEST - SOUTH // 8H
			addLenght( x_centre - y , y_centre - x ); // WEST - NORTH // 10H
			addLenght( x_centre - x , y_centre - y ); // NORTH - WEST // 11H
			if(d >= 2*x){
				d = d - ( 2 * x ) - 1;
				x ++;
			}else if( d < 2 * ( r - y )){
	...
	fillLenght = function(x_centre, y_centre){
		for(var x  in circularArray){
			x = parseInt(x);
			for(var y in circularArray[x]){
				y = parseInt(y);
	...
	andresLenght(rayon, x_centre, y_centre);
	if(fill){
